...
pub use compositor::{Damage, FramePump};
...
/// Which surface rasterizes a window's retained scene.
#[derive(Copy, Clone, Debug, Default, PartialEq, Eq)]
pub enum RendererBackend {
    /// Deterministic CPU Skia plus damaged-region upload. The portable default.
    #[default]
    Cpu,
    /// Skia Ganesh drawing directly into the presenter's wgpu-owned Metal
    /// texture. Experimental and currently available only on macOS.
    DirectGpu,
}
...
pub struct SceneLayer {
    ...
    /// The part of the layer this epoch changed, when the painter can say.
    ///
    /// `None` means *all of it*, which is what a layer says when it does not
    /// know. Saying more is what keeps a keystroke in a text field from
    /// re-rasterizing the page around it: the field's own box changed and every
    /// pixel outside it is the pixel it was, so the compositor redraws the box.
    /// An empty rectangle means the known change was wholly outside the visible
    /// layer and therefore no retained pixel changed.
    ///
    /// It is a promise rather than a hint. A layer that reports a rectangle
    /// smaller than what it actually redrew leaves stale pixels on the screen.
    pub dirty: Option<LayerRect>,
}
...
pub struct WindowConfig {
    ...
    /// Raster backend for the live retained scene.
    ///
    /// Offscreen rendering and deterministic tests always remain on CPU Skia.
...
mod tests {
    ...
    #[test]
    fn cpu_raster_remains_the_window_default() {
        assert_eq!(
            WindowConfig::default().renderer,
            RendererBackend::Cpu,
            "direct GPU stays opt-in until parity and startup gates pass"
        );
    }
}
