...
use super::sizing::InlineRoom;
use super::*;
use crate::{BoxTree, FragmentKind, FragmentTree, build_box_tree};
...
pub(super) fn rect_of(tree: &FragmentTree, boxes: &BoxTree, tag: &str) -> Rect {
...
pub(super) fn boxes_of(tree: &FragmentTree, boxes: &BoxTree, tag: &str) -> Vec<Fragment> {
    tree.iter()
        .filter(|fragment| {
            matches!(fragment.kind, FragmentKind::Box)
                && fragment
...
pub(super) fn laid_out_with_image(
    html: &str,
    width: f32,
    ...
    let images: crate::Images = crate::image_sources(&document, StyleViewport::default())
        ...
        .collect();
    let mut boxes = crate::build_box_tree_with_images(&document, &styles, &images);
    let mut text = otlyra_text::TextEngine::isolated();
    let tree = crate::layout(
        &mut boxes,
        &mut text,
...
pub(super) fn image_rect(tree: &FragmentTree) -> Rect {
    tree.iter()
