...
impl FragmentTree {
    ...
    /// Replace the contents of the fragment `box_id` generated, leaving the box
    /// itself where it is.
    ///
    /// The other half of [`crate::flow::relayout_contained`]: the box did not
    /// move, so only what is inside it is exchanged. The paint order is assigned
    /// again afterwards, because it counts fragments in document order and the
    /// count has changed — that walk touches no text and measures nothing, which
    /// is why a contained relayout is still worth it.
    ///
    /// Returns whether a fragment for `box_id` was found.
    pub fn replace_contents(&mut self, box_id: BoxId, children: Vec<Fragment>) -> bool {
        fn walk(
            fragment: &mut Fragment,
            box_id: BoxId,
            children: &mut Option<Vec<Fragment>>,
        ) -> bool {
            ...
            fragment
                .children
                .iter_mut()
                .any(|child| walk(child, box_id, children))
        }
        let mut children = Some(children);
        if !walk(&mut self.root, box_id, &mut children) {
            return false;
        }
        assign_paint_order(&mut self.root);
        true
    }
