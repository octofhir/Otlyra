...
mod tests {
    ...
    use crate::{ImageResource, PaintOp, RecordingPainter};
    use kurbo::{Affine, Rect, Shape, Stroke};
    use peniko::{
        Blob, Brush, Color, Fill, FontData, ImageAlphaType, ImageData, ImageFormat, ImageSampler,
    };
    use std::sync::Arc;
    ...
    fn every_paintable_item_reaches_the_target() {
        ...
        let shape = Rect::new(0.0, 0.0, 4.0, 4.0).to_path(0.1);
        ...
        list.push_glyphs(
            &FontData::new(Blob::new(Arc::new(b"font")), 0),
            16.0,
            Vec::new(),
        ...
        list.push(DisplayItem::Image {
            image: ImageResource::from(ImageData {
                data: Blob::new(Arc::new(vec![0, 0, 0, 0])),
            ...
            transform: Affine::IDENTITY,
            clip_rect: Some(Rect::new(0.0, 0.0, 1.0, 1.0)),
        });
        ...
        let ops = painter.ops();
        assert!(matches!(ops[0], PaintOp::PushLayer { .. }));
        assert!(matches!(ops[1], PaintOp::Fill { .. }));
        assert!(matches!(ops[2], PaintOp::FillBlurred { .. }));
        assert!(matches!(ops[3], PaintOp::Stroke { .. }));
        assert!(matches!(ops[4], PaintOp::DrawGlyphs { .. }));
        assert!(matches!(ops[5], PaintOp::DrawImage { .. }));
        assert!(matches!(ops[6], PaintOp::PopLayer));
        assert_eq!(ops.len(), 7, "hit-test items must not reach the backend");
