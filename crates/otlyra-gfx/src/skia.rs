...
//!
//! The live painter owns a raster `SkSurface` and hands out its pixels. The
//! bounded macOS candidate can instead wrap a wgpu-owned Metal texture; it keeps
//! the Ganesh context but stores no wgpu type or handle. See
//! `docs/skia-wgpu-interop.md` for the measured adoption gate.
...
pub enum SkiaError {
    ...
    /// Ganesh could not create a Metal recording context from the supplied
    /// device and command queue.
    #[cfg(target_os = "macos")]
    #[error("skia could not create a Metal GPU context")]
    MetalContext,
    /// Ganesh could not wrap the supplied Metal texture as a drawing surface.
    #[cfg(target_os = "macos")]
    #[error("skia could not wrap the {width}x{height} Metal texture")]
    MetalSurface {
    ...
    /// A raw Metal object handle was null.
    #[cfg(target_os = "macos")]
    #[error("the {object} Metal handle was null")]
    NullMetalHandle {
        /// Which required object was missing.
        object: &'static str,
    },
    /// A wrapped external texture cannot be resized in place.
    #[cfg(target_os = "macos")]
    #[error("a wrapped Metal texture must be replaced rather than resized")]
    WrappedMetalResize,
}

/// A [`PaintTarget`] backed by a Skia raster or wrapped Metal surface.
pub struct SkiaPainter {
    // Dropped before `gpu_context`: a GPU surface borrows the direct context.
    surface: sk::Surface,
    /// Keeps a Ganesh context alive for a wrapped GPU surface. Raster surfaces
    /// have no context.
    gpu_context: Option<sk::gpu::DirectContext>,
    width: u32,
    height: u32,
...
impl SkiaPainter {
    ...
    /// Wrap a wgpu-owned Metal texture as this painter's Skia surface.
    ///
    /// This is the narrow interop seam used by the direct-GPU acceptance spike.
    /// It deliberately takes erased Objective-C object pointers rather than
    /// exposing either wgpu or Metal types in this crate's public API.
    ///
    /// # Safety
    ///
    /// - `device`, `queue`, and `texture` must point to live `MTLDevice`,
    ///   `MTLCommandQueue`, and `MTLTexture` objects respectively.
    /// - All three objects must belong to the same Metal device.
    /// - `texture` must be a two-dimensional sRGB RGBA8 render target of exactly
    ///   `width` by `height` pixels.
    /// - The texture must outlive the returned painter and must not be used
    ///   concurrently. Submit subsequent wgpu work only after
    ///   [`SkiaPainter::flush_and_submit`] or [`SkiaPainter::sync_gpu`].
    #[cfg(target_os = "macos")]
    pub unsafe fn wrap_metal_texture(
        ...
        for (object, handle) in [("device", device), ("queue", queue), ("texture", texture)] {
            if handle.is_null() {
                return Err(SkiaError::NullMetalHandle { object });
            }
        }

        // SAFETY: upheld by the caller; Skia retains both Objective-C objects.
        let backend = unsafe { sk::gpu::mtl::BackendContext::new(device, queue) };
        let mut context =
            sk::gpu::direct_contexts::make_metal(&backend, None).ok_or(SkiaError::MetalContext)?;
        // SAFETY: upheld by the caller; TextureInfo retains the Objective-C object.
        let info = unsafe { sk::gpu::mtl::TextureInfo::new(texture) };
        // SAFETY: the dimensions and texture type are part of this method's contract.
        ...
        let surface = sk::gpu::surfaces::wrap_backend_texture(
            &mut context,
            &backend_texture,
        ...
        .ok_or(SkiaError::MetalSurface { width, height })?;
    ...
    /// Submit recorded GPU drawing without waiting for it on the CPU.
    ///
    /// A raster painter has nothing to submit.
    ...
    /// Submit recorded GPU drawing and wait until Metal has completed it.
    ///
    /// The live compositor should prefer queue ordering and avoid this wait. The
    /// acceptance spike uses it before reading pixels for deterministic parity.
    ...
    /// Reallocate a raster surface if the requested size differs.
    ///
    /// Returns `true` if a reallocation happened, so callers can invalidate
    /// anything keyed on surface identity. A wrapped external Metal texture must
    /// be replaced by its owner and returns `WrappedMetalResize`.
    pub fn resize(&mut self, width: u32, height: u32) -> Result<bool, SkiaError> {
        ...
        if self.gpu_context.is_some() {
            return Err(SkiaError::WrappedMetalResize);
        }
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn metal_interop_rejects_null_handles_before_ffi() {
        // SAFETY: null is deliberately passed to exercise the validation that
        // runs before Skia sees any pointer.
        let result = unsafe {
        ...
        assert!(matches!(
            result,
            Err(SkiaError::NullMetalHandle { object: "device" })
        ));
    }
